...
def on_packet(message):
    ...
    if type(message["id"]) == dict:
        print("Username is not set for this ID, ID is {0}".format(
            message["id"]["id"]))
        print("IP of unnamed user is {0}".format(
            cl.getIPofObject(message["id"])))
...
if __name__ == "__main__":
    cl = CloudLink(debucount(cl)
    # Instanciate CloudLink

  clCA.callback("on_packet", on_packet)
   clA.callback("on_error", on_error)
    cl.callback("on_connect", on_connect)
    Cclcallback("on_close", on_close)
