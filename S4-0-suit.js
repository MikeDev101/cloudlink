...
class CloudAccount {
      constructor(runtime, id) {
        ...
        this.cl_onmessage = mWS.onmessage;
        
        mWS.onmessage = (pack) => {
          pack = JSON.parse(pack);
          
          if (pack.hasOwnProperty("cmd") && pack.cmd === "auth")  {
            this.signed_in = true;
            this.token = pack.token;
            this.refresh = pack.refresh_token;
          } 
          
            
          this.cl_onmessage(pack);
        ...
        return {
          ...
          "blocks" : [
            ...
            },
            {
              "opcode": "signup",
              "type": "command",
              "text": "signup with [username] and [password]",
              "arguments": {
                "username": {
                  "type": "String",
                  "text": "Username"
...
class CloudDisk {
  constructor(runtime, id) {
    ...
    this.id = id;
    this.cl_onmessage = mWS.onmessage;
    this.last_gotten_disk = {"data":null, "id":null};
    this.disk_ammount = 0;
    
    mWS.onmessage = (pack) => {
      pack = JSON.parse(pack);
      
      if (pack.hasOwnProperty("cmd") && pack.cmd === "update_disks")  {
        this.disk_ammount = pack.val;
      } else if (pack.hasOwnPropery("cmd") && pack.cmd === "statuscode")  {
        if (pack.hasOwnPropery("disk")) {
          this.last_gotten_disk = pack.disk;
        }  else {
          this.cl_onmessage(pack);
        }
      else {
    ...
    getinfo() {
      return {
        ...
        "blocks" : [
          ...
          {
            "opcode":"gotten_disk_id",
            "type":"reporter",
            "text":"Get disk last disk id"
          },
          {
            "opcode":"get_disk_id",
            "type":"reporter",
    ...
    get_disk_id(args) {
      return this.last_gotten_disk.id;
    }
    get_disk_contents(args) {
      return this.last_gotten_disk.data;
    }
}
class CloudCoin {
  constructor(runtime, id) {
    ...
    mWS.onmessage = (pack) => {
      ...
      if (pack.hasOwnProperty("cmd") && pack.cmd === "update_coins")  {
        this.coins = pack.val;
      } else {
        this.cl_onmessage(pack);
      }
    ...
    return {
      ...
      "blocks" : [
        ...
        },
        {
          "opcode":"give_coins",
          "type":"command",
          "text": "give coins [amount] to [username]",
          "arguments":{
            "amount":{
              "type":"number",
              "text":1
...
(function() {
	var extensionClass = CloudDisk;
	if (typeof window === "undefined" || !window.vm) {
		Scratch.extensions.register(new extensionClass())
    console.log("CloudDisk loaded. Detecting sandboxed mode, performance will suffer. Please load CloudLink in Unsandboxed mode.");
	} else {
		var extensionInstance = new extensionClass(window.vm.extensionManager.runtime);
		var serviceName = window.vm.extensionManager._registerInternalExtension(extensionInstance);
		window.vm.extensionManager._loadedExtensions.set(extensionInstance.getInfo().id, serviceName));
    console.log("CloudDisk 4.0 loaded. Detecting unsandboxed mode.");
	};
})();
(function() {
	var extensionClass = CloudAccount;
	if (typeof window === "undefined" || !window.vm) {
		Scratch.extensions.register(new extensionClass());
    console.log("CloudAccount 4.0 loaded. Detecting sandboxed mode, performance will suffer. Please load CloudLink in Unsandboxed mode.");
	} else {
		var extensionInstance = new extensionClass(window.vm.extensionManager.runtime);
		var serviceName = window.vm.extensionManager._registerInternalExtension(extensionInstance);
		window.vm.extensionManager._loadedExtensions.set(extensionInstance.getInfo().id, serviceName));
    console.log("CloudAccount 4.0 loaded. Detecting unsandboxed mode.");
	};
})();
(function() {
	var extensionClass = CloudCoin;
	if (typeof window === "undefined" || !window.vm) {
		Scratch.extensions.register(new extensionClass());
    console.log("CloudCoin 4.0 loaded. Detecting sandboxed mode, performance will suffer. Please load CloudLink in Unsandboxed mode.");
	} else {
		var extensionInstance = new extensionClass(window.vm.extensionManager.runtime);
		var serviceName = window.vm.extensionManager._registerInternalExtension(extensionInstance);
		window.vm.extensionManager._loadedExtensions.set(extensionInstance.getInfo().id, serviceName));
    console.log("CloudCoin 4.0 loaded. Detecting unsandboxed mode.");
	};
})();
