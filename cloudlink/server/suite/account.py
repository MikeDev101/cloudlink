...
class Account:
    def __init__(self, homeserver_client):
        ...
        self.supporter = self.wss.server.supporter


    # client methods
    async def statuscode(self, _, message, listener):
        ...
        if not ((message["code"] == human_ok) or (message["code_id"] == machine_ok)):
            await self.wss.server.send_packet_unicast(client, "statuscode", val={"code": message["code"], "code_id": message["code_id"]}, listener=listener)
            return
    ...
    async def login(self, client, message, listener):
        ...
        await self.wss.send_packet(cmd="Server_LogClientIn", val={"userid": userid, "user_token": user_token}, quirk=self.wss.server.supporter.quirk_update_msg, listener=f"{userid}")
