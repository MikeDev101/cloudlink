class cl_methods:
    ...
    async def __auto_validate__(self, validator, client, message, listener_detected, listener_id):
        ...
        match validation:
            case self.supporter.invalid:
                ...
                await self.parent.send_code(client, "DataType", listener_detected = listener_detected, listener_id = listener_id)
                return False
            case self.supporter.missing_key:
                # Command syntax is invalid
                await self.parent.send_code(client, "Syntax", listener_detected = listener_detected, listener_id = listener_id)
                return False
            case self.supporter.too_large:
                # Payload size overload
                await self.parent.send_code(client, "TooLarge", listener_detected = listener_detected, listener_id = listener_id)
                return False
        
        return True
    
    async def handshake(self, client, message, listener_detected, listener_id):
    ...
    async def ping(self, client, message, listener_detected, listener_id):
        # Validation is not needed since this command takes no arguments
    ...
    async def gmsg(self, client, message, listener_detected, listener_id):
        # Validate the message syntax and datatypes
        if not await self.__auto_validate__(self.validator[self.gmsg], client, message, listener_detected, listener_id):
            return
        
        # Get rooms
        rooms = set()
        if not "rooms" in message:
        ...
        # Handle listeners
        if listener_detected and (len(rooms) != 0):
            await self.send_packet_unicast(
                client = client,            
                cmd = "gmsg",
                val = message["val"],
    ...
    async def pmsg(self, client, message, listener_detected, listener_id):
        # Validate the message syntax and datatypes
        if not await self.__auto_validate__(self.validator[self.pmsg], client, message, listener_detected, listener_id):
            return
        
        # Check if the client has set their username
        if not client.username_set:
            await self.send_code(client, "IDRequired", listener_detected = listener_detected, listener_id = listener_id)
            return
        ...
        if len(clients) == 0:
            await self.send_code(client, "IDNotFound", listener_detected = listener_detected, listener_id = listener_id)
            return
        ...
        await self.send_code(client, "OK", listener_detected = listener_detected, listener_id = listener_id)
    
    async def gvar(self, client, message, listener_detected, listener_id):
        # Validate the message syntax and datatypes
        if not await self.__auto_validate__(self.validator[self.gvar], client, message, listener_detected, listener_id):
            return
    ...
    async def pvar(self, client, message, listener_detected, listener_id):
        # Validate the message syntax and datatypes
        if not await self.__auto_validate__(self.validator[self.pvar], client, message, listener_detected, listener_id):
            return
    ...
    async def setid(self, client, message, listener_detected, listener_id):
        # Validate the message syntax and datatypes
        if not await self.__auto_validate__(self.validator[self.setid], client, message, listener_detected, listener_id):
            return
        
        if client.username_set:
            await self.send_code(client, "IDSet", listener_detected = listener_detected, listener_id = listener_id)
            return

        result = self.parent.clients.set_username(client, message["val"])
        if result == self.supporter.username_set:
            await self.send_code(client, "OK", extra_data = {"val": self.parent.clients.convert_json(client)}, listener_detected = listener_detected, listener_id = listener_id)
        else:
            await self.send_code(client, "IDConflict", listener_detected = listener_detected, listener_id = listener_id)
    ...
    async def link(self, client, message, listener_detected, listener_id):
        # Validate the message syntax and datatypes
        if not await self.__auto_validate__(self.validator[self.link], client, message, listener_detected, listener_id):
            return
        ...
        if ("default" in client.rooms) and (not "default" in rooms):
    ...
    async def unlink(self, client, message, listener_detected, listener_id):
        # Validate the message syntax and datatypes
        if not await self.__auto_validate__(self.validator[self.unlink], client, message, listener_detected, listener_id):
            return
        ...
        # Prepare the rooms
        rooms = set()
        if (not "val" in message) or (("val" in message) and (len(message["val"]) == 0)):
            # Unlink all rooms
    ...
    async def direct(self, client, message, listener_detected, listener_id):
        # Validate the message syntax and datatypes
        if not await self.__auto_validate__(self.validator[self.direct], client, message, listener_detected, listener_id):
            return
