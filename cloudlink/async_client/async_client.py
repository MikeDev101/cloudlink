...
class multi_client:
    ...
    def run(self):
        ...
        while (len(self.clients_present) + len(self.clients_dead)) != self.clients_counter:
            self.parent.logger.log(f"{(len(self.clients_present) + len(self.clients_dead))} / {self.clients_counter}...")

    def stop(self):
        self.shutdown_flag = True
        self.clients_counter = self.clients_counter - len(self.clients_dead)
        while len(self.clients_present) != 0:
            self.parent.logger.log(f"{len(self.clients_present)} / {self.clients_counter}...", end="\r")
